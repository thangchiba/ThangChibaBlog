...
const MorseSpeedSlider: React.FC<MorseSpeedSliderProps> = ({ wpm, onWpmChange }) => {
  return (
    <div className="flex items-center space-x-2 bg-gray-600 rounded-lg py-1 mt-5">
      <span className="w-5/12 inline-flex items-center px-3 text-gray-900 bg-gray-200 border border-r-0 border-gray-300 rounded-l-md dark:bg-gray-600 dark:text-gray-400 dark:border-gray-600">
        <span className="text-sm md:text-3xl">Speed(WPM) : </span>
        <span className="font-bold text-lg md:text-3xl text-yellow-300"> {wpm}</span>
      </span>
